...
export function EndpointAutocomplete({ endpoints }: EndpointAutocompleteProps) {
  ...
  const filteredEndpoints = React.useMemo(
    () => endpoints.filter(e => e.operationId.toLowerCase().includes(query.toLowerCase())),
    [query, endpoints]
  )
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true); setError(null); setResponse(null);
    const exprs: FilterExpr[] = filterFields
      .filter(f => fieldValues[f.path]?.trim())
      .map(f => ({ fieldPath: f.path, op: fieldOps[f.path] || 'Eq', value: f.type === 'integer' || f.type === 'number' ? Number(fieldValues[f.path]) : fieldValues[f.path] }));
                ...
                <div key={f.path} className="flex flex-col">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      ...
                      <SelectContent>
                        {FilterOperators.map(o=><SelectItem key={o} value={o}>{o}</SelectItem>)}
                      </SelectContent>
                    </Select>
                    <Input
